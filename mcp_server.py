...
nature). Auth comes from secrets/cookies.json (see scripts/extract_cookies.py).
...
from banana.client import get_client
from banana.prompts import load_prompt_file, select
from banana.generator import generate, generate_prompt
...
async def banana_account_status() -> str:
    ...
    'signed out' result means secrets/cookies.json needs a fresh export.
    """
    try:
...
async def banana_generate_image(prompt: str, out_dir: str | None = None,
    ...
    Args:
        ...
        out_dir: Directory to save into (default: <project>/output/adhoc).
